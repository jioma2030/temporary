...
def load_data():
    try:
        # GitHub Raw URL로 CSV 로드
        csv_url = "https://raw.githubusercontent.com/yourusername/yourrepo/main/전북특별자치도_대기오염정보(이산화질소_일산화탄소)_20200331.csv"
        ...
        # 결측치 확인
        st.write("결측치 확인 (위도, 경도, 일산화탄소):", df[["위도", "경도", "일산화탄소"]].isna().sum())
...
if data.empty:
    st.error("데이터가 비어 있습니다. CSV 파일 또는 측정소 이름을 확인하세요.")
else:
    # 지도 생성
